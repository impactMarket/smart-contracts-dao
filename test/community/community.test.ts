...
// 		(await communityInstance.previousCommunity()).should.be.equal(zeroAddress);
// 		(await communityInstance.claimAmount()).should.be.equal(claimAmountTwo.toString());
// 		(await communityInstance.baseInterval()).should.be.equal(day.toString());
// 		(await communityInstance.incrementInterval()).should.be.equal(hour.toString());
// 		(await communityInstance.maxClaim()).should.be.equal(maxClaimTen.toString());
// 		(await communityInstance.validBeneficiaryCount()).should.be.equal(0);
// 		(await communityInstance.treasuryFunds()).should.be.equal(parseEther("100"));
// 		(await communityInstance.privateFunds()).should.be.equal(0);
// 		(await communityInstance.communityAdmin()).should.be.equal(communityAdminInstance.address);
