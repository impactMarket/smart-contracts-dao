...
import {
    createPool, getExactInput,
    getExchangePath, increaseLiquidity, position, swap,
...
describe("Treasury", () => {
    ...
    describe("Treasury - new tokens", () => {
        ...
        async function test(lp1Amount: BigNumber, lp2Amount: BigNumber, lpToAdd: BigNumber, isHalf: boolean) {
            console.log('******************************************************************************************')
            console.log(fromEther(lp1Amount))
            console.log(fromEther(lp2Amount))
            console.log(fromEther(lpToAdd))
            console.log(isHalf)

            await cUSD.mint(user3.address, lp1Amount.add(lpToAdd));
            await mUSD.mint(user3.address, lp2Amount);

            // console.log('after mint');
            // console.log(fromEther(await cUSD.balanceOf(user3.address)))
            // console.log(fromEther(await mUSD.balanceOf(user3.address)))

            await createPool(user3, cUSD, mUSD, lp1Amount, lp2Amount, Treasury.address);
            ...
            const amountOut0 = await getExactInput(cUSD, mUSD, lpToAdd.div(2));
            const amountOut1 = await getExactInput(mUSD, cUSD, amountOut0);

            // console.log('amountOut0: ', amountOut0);
            // console.log('amountOut1: ', amountOut1);

            const amountToSwap = isHalf ? lpToAdd.div(2) : (lpToAdd.add(amountOut1).sub(amountOut0)).div(2);

            await swap(user3, cUSD, mUSD, amountToSwap);

            const balance1AfterSwap = await cUSD.balanceOf(user3.address);
            const balance2AfterSwap = await mUSD.balanceOf(user3.address);
            // console.log('after swap');
            // console.log(fromEther(balance1AfterSwap));
            // console.log(fromEther(balance2AfterSwap));

            await increaseLiquidity(user3, 2535, cUSD, mUSD, balance1AfterSwap, balance2AfterSwap);

            console.log('after increase lp');
            console.log(fromEther(await cUSD.balanceOf(user3.address)))
            console.log(fromEther(await mUSD.balanceOf(user3.address)))
        }
        ...
        it("Should not set token if not owner", async function () {
            await expect(
                Treasury.connect(user1).setToken(mUSD.address, toEther(0.5), LpStrategy.MainCoin, 0, '0x', '0x')
            ).to.be.rejectedWith(
                "Treasury: caller is not the owner nor ImpactMarketCouncil"
            );
        ...
        it("Should setToken without exchangePath", async function () {
            await expect(
                Treasury.setToken(mUSD.address, toEther(0.5), LpStrategy.NONE, 0, "0x", "0x")
            ).to.be.fulfilled;
        ...
        it("Should not set token without rate", async function () {
            ...
            await expect(
                Treasury.setToken(mUSD.address, 0, LpStrategy.NONE, 0, "0x", "0x")
        ...
        it("Should not set token with invalid exchangePathToCUSD #1", async function () {
            ...
            await expect(
                Treasury.setToken(mUSD.address, toEther(1), LpStrategy.NONE, 0, exchangePath, "0x")
            ).to.be.rejectedWith(
        ...
        it("Should not set token with invalid exchangePathToPACT #1", async function () {
            ...
            await expect(
                Treasury.setToken(mUSD.address, toEther(1), LpStrategy.NONE, 0, '0x', exchangePath)
            ).to.be.rejectedWith(
                "Transaction reverted without a reason string"
            );
        ...
        it("Should not remove token if not owner", async function () {
            await Treasury.setToken(mUSD.address, toEther(0.5), LpStrategy.NONE, 0, "0x", "0x");
    ...
    describe("Treasury + TreasuryLpSwap - basic", () => {
        ...
        it("Should update UniswapRouter if owner", async function () {
            await expect(TreasuryLpSwap.updateUniswapRouter(FAKE_ADDRESS))
                .to.be.fulfilled;
            expect(await TreasuryLpSwap.uniswapRouter()).to.be.equal(FAKE_ADDRESS);
        });
        ...
        it("Should update uniswapQuoter if owner", async function () {
            await expect(TreasuryLpSwap.updateUniswapQuoter(FAKE_ADDRESS))
                .to.be.fulfilled;
            expect(await TreasuryLpSwap.uniswapQuoter()).to.be.equal(FAKE_ADDRESS);
        });
        ...
        it("Should not update uniswapNFTPositionManager if not owner", async function () {
            ...
            expect(await TreasuryLpSwap.uniswapNFTPositionManager()).to.be.equal(
                uniswapNFTPositionManagerAddress
            );
        });

        it("Should update uniswapNFTPositionManager if owner", async function () {
            await expect(TreasuryLpSwap.updateUniswapNFTPositionManager(FAKE_ADDRESS))
                .to.be.fulfilled;
            expect(await TreasuryLpSwap.uniswapNFTPositionManager()).to.be.equal(
                FAKE_ADDRESS
            );
        ...
        it("Should set token with uniswapTokenId if owner", async function () {
            ...
            expect(token.exchangePathToCUSD).to.be.equal(exchangePathToCUSD);
            expect(token.exchangePathToPACT).to.be.equal(exchangePathToPACT);
            expect(token.uniswapNFTPositionManagerId).to.be.equal(mUSDToPACTTokenId);
            expect(await Treasury.tokenListLength()).to.be.equal(2);
            expect(await Treasury.tokenListAt(0)).to.be.equal(cUSD.address);
            expect(await Treasury.tokenListAt(1)).to.be.equal(mUSD.address);
        });
        ...
        it("Should not set token with invalid uniswapTokenId", async function () {
            ...
            ).to.be.to.rejectedWith("Treasury::setToken: invalid uniswapNFTPositionManagerId")
        });

        it("Should transferToTreasury without uniswapNFTPositionManagerId #1", async function () {
            const initialTreasuryCUSDBalance = await cUSD.balanceOf(Treasury.address);

            await cUSD.approve(Treasury.address, toEther(1))

            await expect(
                Treasury.transferToTreasury(
                    cUSD.address,
                    toEther(1),
                )
            ).to.be.to.fulfilled;

            expect(await cUSD.balanceOf(Treasury.address)).to.be.equal(initialTreasuryCUSDBalance.add(toEther(1)));
        });

        it("Should transferToTreasury without uniswapNFTPositionManagerId #2", async function () {
            const initialTreasuryMUSDBalance = await mUSD.balanceOf(Treasury.address);
            ...
            expect(await mUSD.balanceOf(Treasury.address)).to.be.equal(initialTreasuryMUSDBalance.add(toEther(1)));
        });
    });
    ...
    describe("Treasury + TreasuryLpSwap - increase lp", () => {
        ...
        it("Should transferToTreasury with uniswapNFTPositionManagerId (cUSD, small amount)", async function () {
            ...
            await expect(
                ...
                .to.be.to.emit(TreasuryLpSwap, "LiquidityIncreased")
                .withArgs(
                    cUSDToPACTTokenId,
