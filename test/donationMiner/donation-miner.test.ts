...
describe.only("DonationMiner", () => {
  ...
  let recurringCron: SignerWithAddress;
  ...
  let user1InitialCUSDBalance = toEther(1000000);
  let user2InitialCUSDBalance = toEther(10000000);
  let user3InitialCUSDBalance = toEther(100000000);
  let user4InitialCUSDBalance = toEther(100000000);
  ...
  const deploy = deployments.createFixture(async () => {
    ...
    await cUSD.mint(user1.address, user1InitialCUSDBalance);
    await cUSD.mint(user2.address, user2InitialCUSDBalance);
    await cUSD.mint(user3.address, user3InitialCUSDBalance);
    await cUSD.mint(user4.address, user4InitialCUSDBalance);
    ...
    await DonationMiner.updateRecurringCronAddress(recurringCron.address);
  });
  ...
  describe("Donation Miner (claimDelay = 0, againstPeriods = 0)", () => {
    ...
    it("Should have correct values", async function () {
      expect(await DonationMiner.getVersion()).to.be.equal(5);
      ...
      expect(await DonationMiner.airdropV3()).to.equal(airdropV3Mock.address);
      expect(await DonationMiner.microcredit()).to.equal(microcreditMock.address);
      expect(await DonationMiner.recurringCronAddress()).to.equal(recurringCron.address);
    });
    ...
    it("Should approve and donateFrom 100 cUSD from user1", async function () {
      const user1Donation = toEther("200");
  ...
  xdescribe("Donation Miner + Community", () => {
  ...
  describe("Donation Miner (donateVirtual)", () => {
    ...
    it("Should updateMicrocredit if admin", async function () {
      expect(await DonationMiner.microcredit()).to.be.equal(microcreditMock.address);
      await expect(DonationMiner.updateMicrocredit(user4.address)).to.be.fulfilled;
      expect(await DonationMiner.microcredit()).to.be.equal(user4.address);
    });

    it("Should not updateMicrocredit if not admin", async function () {
      expect(await DonationMiner.microcredit()).to.be.equal(microcreditMock.address);
      await expect(
        DonationMiner.connect(user1).updateMicrocredit(user4.address)
      ).to.be.rejectedWith("Ownable: caller is not the owner");
      expect(await DonationMiner.microcredit()).to.be.equal(microcreditMock.address);
    });

    it("Should updateAirdropV3 if admin", async function () {
      expect(await DonationMiner.airdropV3()).to.be.equal(airdropV3Mock.address);
      await expect(DonationMiner.updateAirdropV3(user4.address)).to.be.fulfilled;
      expect(await DonationMiner.airdropV3()).to.be.equal(user4.address);
    });

    it("Should not updateAirdropV3 if not admin", async function () {
      expect(await DonationMiner.airdropV3()).to.be.equal(airdropV3Mock.address);
      await expect(
        DonationMiner.connect(user1).updateAirdropV3(user4.address)
      ).to.be.rejectedWith("Ownable: caller is not the owner");
      expect(await DonationMiner.airdropV3()).to.be.equal(airdropV3Mock.address);
    });
  ...
  describe("Donation Miner (donateFrom)", () => {
    before(async function () {
    });
    ...
    it("Should updateRecurringCronAddress if admin", async function () {
      expect(await DonationMiner.recurringCronAddress()).to.be.equal(recurringCron.address);
      await expect(DonationMiner.updateRecurringCronAddress(user4.address)).to.be.fulfilled;
      expect(await DonationMiner.recurringCronAddress()).to.be.equal(user4.address);
    });

    it("Should not updateRecurringCronAddress if not admin", async function () {
      expect(await DonationMiner.recurringCronAddress()).to.be.equal(recurringCron.address);
      await expect(
        DonationMiner.connect(user1).updateRecurringCronAddress(user4.address)
      ).to.be.rejectedWith("Ownable: caller is not the owner");
      expect(await DonationMiner.recurringCronAddress()).to.be.equal(recurringCron.address);
    });
    ...
    it("Should donateVirtual if recurringCron", async function () {
      const user1DonationAmount = toEther("100");
      const user1ExpectedReward = toEther("4320000");

      await cUSD.connect(user1).approve(DonationMiner.address, user1DonationAmount);

      await expect(
        DonationMiner.connect(recurringCron).donateFrom(
          user1.address,
          cUSD.address,
          user1DonationAmount,
          user1.address
      ...
      expect(
        (await DonationMiner.rewardPeriods(1)).donationsAmount
      ).to.be.equal(user1DonationAmount);

      const donation1 = await DonationMiner.donations(1);
      expect(donation1.donor).to.equal(user1.address);
      expect(donation1.target).to.equal(Treasury.address);
      expect(donation1.rewardPeriod).to.equal(1);
      expect(donation1.amount).to.equal(user1DonationAmount);
      expect(donation1.token).to.equal(cUSD.address);
      expect(donation1.initialAmount).to.equal(
        user1DonationAmount
      );
      ...
      expect(await cUSD.balanceOf(user1.address)).to.equal(
        user1InitialCUSDBalance.sub(user1DonationAmount)
      );
    ...
    it("Should donateFrom and donate have the same result", async function () {
      const user1DonationAmount = toEther("100");
      const user1ExpectedReward = toEther("4320000").div(2);
      ...
      expect(
        (await DonationMiner.rewardPeriods(1)).donationsAmount
      ).to.be.equal(user1DonationAmount.add(user2DonationAmount));
