...
import {
    advanceNSeconds, advanceNSecondsAndBlock, advanceTimeAndBlock,
...
describe.only("Microcredit", () => {
    ...
    const deploy = deployments.createFixture(async () => {
        await deployments.fixture("MicrocreditTest", {fallbackToGlobal: false});
    ...
    describe("Microcredit - basic", () => {
        ...
        it("should have correct values", async function () {
            ...
            (await Microcredit.revenueAddress()).should.eq(MicrocreditRevenue.address);

            await (Microcredit.userLoans(user1.address, 0)).should.be.rejectedWith('Microcredit: Invalid wallet address');
        });

        it("Should update revenueAddress if owner", async function () {
            (await Microcredit.revenueAddress()).should.eq(MicrocreditRevenue.address);
            await (Microcredit.connect(owner).updateRevenueAddress(user1.address)).should.be.fulfilled;
            (await Microcredit.revenueAddress()).should.eq(user1.address);
        });

        it("Should not update revenueAddress if not owner", async function () {
            await (
                Microcredit.connect(user1).updateRevenueAddress(user1.address)
            ).should.be.rejectedWith("Ownable: caller is not the owner");
        });

        it("Should addManagers if owner #1", async function () {
            (await Microcredit.managerListLength()).should.eq(0);
            await (Microcredit.connect(owner).addManagers([manager1.address])).should.be.fulfilled;
        ...
        it("Should addManagers if owner #2", async function () {
            (await Microcredit.managerListLength()).should.eq(0);
            await (Microcredit.connect(owner).addManagers([manager1.address, manager2.address])).should.be.fulfilled;
        ...
        it("Should not addManagers if not owner", async function () {
            await (
                Microcredit.connect(user1).addManagers([manager1.address])
            ).should.be.rejectedWith("Ownable: caller is not the owner");
        });

        it("Should removeManagers if owner #1", async function () {
            await (Microcredit.connect(owner).addManagers([manager1.address])).should.be.fulfilled;
            await (Microcredit.connect(owner).removeManagers([manager1.address])).should.be.fulfilled;
        ...
        it("Should removeManagers if owner #2", async function () {
            await (Microcredit.connect(owner).addManagers([manager1.address, manager2.address])).should.be.fulfilled;
            await (Microcredit.connect(owner).removeManagers([manager1.address, manager2.address])).should.be.fulfilled;
        ...
        it("Should removeManagers if owner #3", async function () {
            await (Microcredit.connect(owner).addManagers([manager1.address, manager2.address])).should.be.fulfilled;
            await (Microcredit.connect(owner).removeManagers([manager2.address])).should.be.fulfilled;
        ...
        it("Should not removeManagers if not owner", async function () {
            await (
                Microcredit.connect(user1).removeManagers([manager1.address])
            ).should.be.rejectedWith("Ownable: caller is not the owner");
        });
        ...
        it("Should transferERC20 to address if owner", async function () {
            ...
            (await cUSD.balanceOf(Microcredit.address))
                .should.eq(initialMicrocreditBalance.sub(toEther(100)));
            (await cUSD.balanceOf(user1.address))
                .should.eq(initialUserBalance.add(toEther("100")));
        });

        it("Should not transferERC20 if not owner", async function () {

            await Microcredit.connect(deployer).transferERC20(
                cUSD.address,
                user1.address,
                toEther("100")
            ).should.be.rejectedWith('Ownable: caller is not the owner');
    ...
    describe("Microcredit - loan functionalities (revenue address = 0)", () => {
        ...
        beforeEach(async () => {
            ...
            await Microcredit.connect(owner).updateRevenueAddress(ethers.constants.AddressZero);
            await Microcredit.connect(owner).addManagers([manager1.address]);
        });

        it("Should not addLoan if not manager", async function () {
            await (
                Microcredit.connect(user1).addLoan(user1.address, toEther(100), sixMonth, toEther(0.2))
        ...
        it("should addLoan if manager", async function () {
            ...
            let walletMetadata = await Microcredit.walletMetadata(user1.address);
            ...
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance);
        });
        ...
        it("should addLoan twice for same user", async function () {
            ...
            await (Microcredit.connect(manager1).addLoan(user1.address, amount1, period1, dailyInterest1))
                .should.emit(Microcredit, 'LoanAdded')
                .withArgs(user1.address, 0, amount1, period1, dailyInterest1);
            await (Microcredit.connect(manager1).addLoan(user1.address, amount2, period2, dailyInterest2))
                .should.emit(Microcredit, 'LoanAdded')
                .withArgs(user1.address, 1, amount2, period2, dailyInterest2);

            let walletMetadata1 = await Microcredit.walletMetadata(user1.address);
        ...
        it("should addLoan for multiple users", async function () {
            ...
            await (Microcredit.connect(manager1).addLoan(user1.address, amount1, period1, dailyInterest1)).should.be.fulfilled;
            await (Microcredit.connect(manager1).addLoan(user2.address, amount2, period2, dailyInterest2)).should.be.fulfilled;
            ...
            let walletMetadata2 = await Microcredit.walletMetadata(user2.address);
        ...
        it("Should not addLoans if not manager", async function () {
            await (
                Microcredit.connect(user1).addLoans([user1.address], [toEther(100)], [sixMonth], [toEther(0.2)])
            ).should.be.rejectedWith("Microcredit: caller is not a manager");
        });
        ...
        it("should addLoans twice for same user", async function () {
            ...
            await (Microcredit.connect(manager1).addLoans(
        ...
        it("should claimLoan #1", async function () {
            ...
            await (Microcredit.connect(manager1).addLoan(user1.address, amount, period, dailyInterest)).should.be.fulfilled;
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount));
        });
        ...
        it("should claimLoan #2", async function () {
            ...
            await (Microcredit.connect(manager1).addLoan(user2.address, amount, period, dailyInterest)).should.be.fulfilled;
            await (Microcredit.connect(manager1).addLoan(user1.address, amount, period, dailyInterest)).should.be.fulfilled;
        ...
        it("should claimLoan #3", async function () {
            ...
            await (Microcredit.connect(manager1).addLoan(user1.address, amount1, period1, dailyInterest1)).should.be.fulfilled;
            await (Microcredit.connect(manager1).addLoan(user1.address, amount2, period2, dailyInterest2)).should.be.fulfilled;
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount2));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount2));
        });
        ...
        it("should not claimLoan if invalid loan id", async function () {
            ...
            await (Microcredit.connect(user1).claimLoan(1)).should.be.rejectedWith('Microcredit: Loan doesn\'t exist');
            await (Microcredit.connect(user2).claimLoan(0)).should.be.rejectedWith('Microcredit: Invalid wallet address');
        });
        ...
        it("should repayLoan (interest=0)", async function () {
            ...
            await cUSD.connect(user1).approve(Microcredit.address, repaymentAmount1);

            await (Microcredit.connect(user1).repayLoan(0, repaymentAmount1))
                .should.emit(Microcredit, 'RepaymentAdded')
                .withArgs(user1.address, 0, repaymentAmount1, amount.sub(repaymentAmount1));
            ...
            let repayment = await Microcredit.userLoanRepayments(user1.address, 0, 0);
            (repayment.amount).should.eq(repaymentAmount1);
            (repayment.date).should.eq(repaymentDate);

            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(repaymentAmount1));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount).add(repaymentAmount1));
        });
        ...
        it("should repayLoan multiple times (interest=0)", async function () {
            ...
            await cUSD.connect(user1).approve(Microcredit.address, repaymentAmount1.add(repaymentAmount2));

            await (Microcredit.connect(user1).repayLoan(0, repaymentAmount1)).should.be.fulfilled;
            const repaymentDate1 = await getCurrentBlockTimestamp();
            await (Microcredit.connect(user1).repayLoan(0, repaymentAmount2)).should.be.fulfilled;
            ...
            (loan.dailyInterest).should.eq(dailyInterest);
            (loan.startDate).should.eq(statDate);
            (loan.lastComputedDebt).should.eq(amount.sub(repaymentAmount1.add(repaymentAmount2)));
            (loan.currentDebt).should.eq(amount.sub(repaymentAmount1.add(repaymentAmount2)));
            ...
            let repayment1 = await Microcredit.userLoanRepayments(user1.address, 0, 0);
            (repayment1.amount).should.eq(repaymentAmount1);
            (repayment1.date).should.eq(repaymentDate1);

            let repayment2 = await Microcredit.userLoanRepayments(user1.address, 0, 1);
            (repayment2.amount).should.eq(repaymentAmount2);
            (repayment2.date).should.eq(repaymentDate2);

            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(repaymentAmount1.add(repaymentAmount2)));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount).add(repaymentAmount1.add(repaymentAmount2)));
        });
        ...
        it("should changeUserAddress if manager", async function () {
            ...
            await (Microcredit.connect(manager1).changeUserAddress(user1.address, user2.address))
            ...
            await (Microcredit.userLoanRepayments(user1.address, 0, 0)).should.be.rejectedWith('Microcredit: Invalid wallet address');
            repayment = await Microcredit.userLoanRepayments(user2.address, 0, 0);
            (repayment.amount).should.eq(repaymentAmount1);
            (repayment.date).should.eq(repaymentDate);

            await (Microcredit.connect(user1).repayLoan(0, repaymentAmount1)).should.be.rejectedWith('Microcredit: Invalid wallet address');
            await cUSD.connect(user2).approve(Microcredit.address, repaymentAmount2);
            await (Microcredit.connect(user2).repayLoan(0, repaymentAmount2)).should.be.fulfilled;
            ...
            repayment = await Microcredit.userLoanRepayments(user2.address, 0, 1);
        ...
        it("should not changeUserAddress if not user", async function () {
            await (Microcredit.connect(manager1).changeUserAddress(user1.address, user2.address))
                .should.be.rejectedWith('Microcredit: This user cannot be moved');
        });
        ...
        it("should not changeUserAddress if target address is user", async function () {
            ...
            await (Microcredit.connect(manager1).changeUserAddress(user1.address, user2.address))
                .should.be.rejectedWith('Microcredit: Target wallet address is invalid');
        });

        it("should not changeUserAddress if not manager", async function () {
            await (Microcredit.connect(user1).changeUserAddress(user1.address, user2.address))
                .should.be.rejectedWith('Microcredit: caller is not a manager');
        });
        ...
        it("should repayLoan after 1 day", async function () {
            ...
            await cUSD.connect(user1).approve(Microcredit.address, expectedCurrentDebt);

            await (Microcredit.connect(user1).repayLoan(0, expectedCurrentDebt)).should.be.fulfilled;
            const repaymentDate1 = await getCurrentBlockTimestamp();
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(expectedCurrentDebt));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount).add(expectedCurrentDebt));
        });
        ...
        it("should not repay more than current debt", async function () {
            ...
            await cUSD.connect(user1).approve(Microcredit.address, repaymentAmount);

            await (Microcredit.connect(user1).repayLoan(0, repaymentAmount))
                .should.emit(Microcredit, 'RepaymentAdded')
        ...
        it("should not repayLoan after fully repayed", async function () {
            ...
            await (Microcredit.connect(user1).repayLoan(0, expectedCurrentDebt))
                .should.be.rejectedWith('Microcredit: Loan has already been fully repayed');
        });
        ...
        it("should not repayLoan if invalid wallet address", async function () {
            ...
            await (Microcredit.connect(user1).repayLoan(1, amount))
        ...
        it("should repayLoan multiple times in same day", async function () {
            ...
            (loan.dailyInterest).should.eq(dailyInterest);
            (loan.startDate).should.eq(statDate);
            (loan.lastComputedDebt).should.eq(expectedCurrentDebt.sub(repaymentAmount1));
        ...
        it("should repayLoan multiple times in multiple days", async function () {
            ...
            (loan.dailyInterest).should.eq(dailyInterest);
            (loan.startDate).should.eq(statDate);
            (loan.lastComputedDebt).should.eq(expectedCurrentDebt1.sub(repaymentAmount1));
            (loan.currentDebt).should.eq(expectedCurrentDebt1.sub(repaymentAmount1));
            (loan.amountRepayed).should.eq(repaymentAmount1);
            (loan.repaymentsLength).should.eq(1);
            (loan.lastComputedDate).should.eq(repaymentDate1);
            ...
            (loan.dailyInterest).should.eq(dailyInterest);
            (loan.startDate).should.eq(statDate);
            (loan.lastComputedDebt).should.eq(expectedCurrentDebt2.sub(repaymentAmount2));
            (loan.currentDebt).should.eq(expectedCurrentDebt2.sub(repaymentAmount2));
            (loan.amountRepayed).should.eq(repaymentAmount1.add(repaymentAmount2));
            (loan.repaymentsLength).should.eq(2);
            (loan.lastComputedDate).should.eq(repaymentDate2);
    ...
    describe("Microcredit - loan functionalities (revenue address != 0)", () => {
        ...
        it("should repayLoan and redirect revenue #1", async function () {
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(expectedCurrentDebt));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance);
            (await cUSD.balanceOf(MicrocreditRevenue.address)).should.eq(expectedCurrentDebt.sub(amount));
        });
        ...
        it("should repayLoan and redirect revenue #2", async function () {
            ...
            await (Microcredit.connect(user1).repayLoan(0, toEther(40))).should.be.fulfilled;
            const repaymentDate1 = await getCurrentBlockTimestamp();
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(toEther(40)));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance.sub(amount).add(toEther(40)));
            ...
            await (Microcredit.connect(user1).repayLoan(0, toEther(60))).should.be.fulfilled;
            const repaymentDate2 = await getCurrentBlockTimestamp();
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(toEther(100)));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance);
            (await cUSD.balanceOf(MicrocreditRevenue.address)).should.eq(0);

            await cUSD.connect(user1).approve(Microcredit.address, toEther('43.281419556037096897'));
            await (Microcredit.connect(user1).repayLoan(0, toEther('43.281419556037096897'))).should.be.fulfilled;
            ...
            let repayment3 = await Microcredit.userLoanRepayments(user1.address, 0, 2);
            (repayment3.amount).should.eq(toEther('43.281419556037096897'));
            (repayment3.date).should.eq(repaymentDate3);

            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(expectedCurrentDebt));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance);
            (await cUSD.balanceOf(MicrocreditRevenue.address)).should.eq(toEther('43.281419556037096897'));
        });
        ...
        it("should repayLoan and redirect revenue #3", async function () {
            ...
            await (Microcredit.connect(user1).repayLoan(0, toEther(70))).should.be.fulfilled;
            const repaymentDate2 = await getCurrentBlockTimestamp();
            ...
            (await cUSD.balanceOf(user1.address)).should.eq(initialUser1Balance.add(amount).sub(toEther(110)));
            (await cUSD.balanceOf(Microcredit.address)).should.eq(initialMicrocreditBalance);
            (await cUSD.balanceOf(MicrocreditRevenue.address)).should.eq(toEther(10));

            await cUSD.connect(user1).approve(Microcredit.address, toEther('33.281419556037096897'));
            await (Microcredit.connect(user1).repayLoan(0, toEther('33.281419556037096897'))).should.be.fulfilled;
            const repaymentDate3 = await getCurrentBlockTimestamp();
