...
describe.only("Microcredit", () => {
	...
	describe.only("Microcredit - editLoan", () => {
		const manager1currentLentAmountLimit = toEther(1000);
		...
		it.only("Should editLoan just after a repayment", async function () {
			const amount = toEther(100);
			const period = sixMonth;
			const dailyInterest = toEther(0.2);
			const claimDeadline = (await getCurrentBlockTimestamp()) + 1000;
			...
			loanAfter2.lastComputedDate.should.eq(statDate);
			loanAfter2.currentDebt.should.eq(getDebtOnDayX(newLastComputedDebt, newDailyInterest, 0));
		});
	});
});
