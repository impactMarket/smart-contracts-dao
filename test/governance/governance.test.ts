...
describe("IPCTGovernator", function() {
  ...
  beforeEach(async function () {
    ...
    communityAdmin = await CommunityAdmin.deploy(testToken1.address, owner.address);
    communityFactory = await CommunityFactory.deploy(testToken1.address, communityAdmin.address);
  ...
  it("should create community", async function() {
    ...
    const values = [0];
    const signatures = ['addCommunity(address,uint256,uint256,uint256,uint256)'];
    ...
    await expect(ipctDelegator.propose(targets, values, signatures, calldatas, descriptions)).to.be.fulfilled;

    await advanceNBlocks(1);

    await expect(ipctDelegator.castVote(1, 1)).to.be.fulfilled;
    await expect(ipctDelegator.connect(alice).castVote(1, 1)).to.be.fulfilled;
