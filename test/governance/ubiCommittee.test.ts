...
// 		await communityAdmin.transferOwnership(ubiCommittee.address);
...
// 		await expect(
// 			ubiCommittee.connect(owner).propose(
// 				// targets,
// 				// values,
// 				signatures,
// 				calldatas,
...
// 		await expect(ubiCommittee.connect(owner).castVote(1, 1)).to.be
// 			.fulfilled;
...
// 		await expect(ubiCommittee.connect(owner).execute(1)).to.be.fulfilled;
// 	});
...
// 		// await expect(ubiCommittee.castVote(1, 1)).to.be.fulfilled;
...
// 		await expect(ubiCommittee.castVote(2, 1)).to.be.fulfilled;
...
// 		// await expect(
// 		// 	ubiCommittee.setProposalThreshold(PROPOSAL_THRESHOLD.mul(2))
// 		// ).to.be.fulfilled;
// 	});
...
// 		// ).to.be.rejectedWith("Ownable: caller is not the owner");
// 		await expect(
// 			ubiCommittee.connect(alice).setQuorumVotes(0)
// 		).to.be.rejectedWith("Ownable: caller is not the owner");
// 		// await expect(
